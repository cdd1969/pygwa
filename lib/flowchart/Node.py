# -*- coding: utf-8 -*-
"""
This block is the reimplementation of Node module from pyqtgraph,
...
Original file: pyqtgraph/flowchart/Node.py
"""
...
import numpy as np
from pyqtgraph.flowchart.eq import eq


def strDict(d):
    """
    create dictionary from tuples (a, b)
    """
...
class Node(QtCore.QObject):
    ...
    def __init__(self, name, terminals=None, allowAddInput=False, allowAddOutput=False, allowRemove=True):
        ...
        **Arguments:**
        ...
        if terminals is None:
            print 'Warning. Creating a node without terminals. Finishing construction of Node @%x' %id(self)
            return
    ...
    def nextTerminalName(self, name):
        ...
        i.e. if user is passing name='dataInput' and curent node has a terminal with such a name,
        this function will return 'dataInput.1' (if this name is not existing, if it is - will return 'dataInput.2'... etc.)"""
        name2 = name
        i = 1
    ...
    def addTerminal(self, name, **opts):
        """Generate a new Terminal and Add it to this Node with the given name. Extra
        keyword arguments are passed to Terminal.__init__.
                
        Causes sigTerminalAdded to be emitted."""
        print 'adding terminal'
        name = self.nextTerminalName(name)
        print 'adding terminal. name'
        term = Terminal(self, name, **opts)
        print 'adding terminal. term'
    ...
    def graphicsItem(self):
        """Return the GraphicsItem for this node. This object is displayen in a flowchart canvas
        Subclasses may re-implement this method to customize their appearance in the flowchart."""

        print 'graphicsItem() called'
        
        if self._graphicsItem is None:
            print 'graphicsItem() creating graphicsItem'
            self._graphicsItem = NodeGraphicsItem(self)
            print 'graphicsItem() creating graphicsItem. ok'
...
class NodeGraphicsItem(GraphicsObject):
    def __init__(self, node):
        print 'NodeGraphicsItem() initing'
        GraphicsObject.__init__(self)
        print 'GraphicsObject() inited.ok'
        ...
        self.node = node
        print 'NodeGraphicsItem() initing. flags'
        ...
        print type(self)
        a = QtGui.QGraphicsTextItem('hallo', parent=None)
        print isinstance(self, QtGui.QGraphicsItem)
        self.nameItem = QtGui.QGraphicsTextItem('self.node.name()', 0)
        print 'NodeGraphicsItem() initing. QGraphicsTextItem().ok'
        self.nameItem.setDefaultTextColor(QtGui.QColor(50, 50, 50))
        self.nameItem.moveBy(self.bounds.width()/2. - self.nameItem.boundingRect().width()/2., 0)
        self.nameItem.setTextInteractionFlags(QtCore.Qt.TextEditorInteraction)
        print 'NodeGraphicsItem() initing. updating terms'
        ...
        self.menu = None
        print 'NodeGraphicsItem() initing.building menu'
        self.buildMenu()
        print 'NodeGraphicsItem() initing.building menu.ok'
    ...
    def mouseClickEvent(self, ev):
        ...
        if int(ev.button()) == int(QtCore.Qt.LeftButton):
            ...
            sel = self.isSelected()
            #ret = QtGui.QtGui.QGraphicsItem.mousePressEvent(self, ev)
