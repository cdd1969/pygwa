...
class divideNode(NodeWithCtrlWidget):
    """Divide all values in A-column `a` by a constant `c`. Divide itemwise A-column `a` by B-column `b`"""
    nodeName = "Divide"
    uiTemplate = [
            {'title': 'Equation', 'name': 'eq', 'type': 'str', 'value': 'a = a / b / c', 'readonly': True},
            {'title': 'a', 'name': 'a', 'type': 'list', 'value': None, 'default': None, 'values': [None], 'tip': 'Name of the column in A-terminal that will be divided'},
    ...
    def _createCtrlWidget(self, **kwargs):
        return divideNodeCtrlWidget(**kwargs)
...
class divideNodeCtrlWidget(NodeCtrlWidget):
    def __init__(self, **kwargs):
        super(divideNodeCtrlWidget, self).__init__(**kwargs)
